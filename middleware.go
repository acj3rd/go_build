	...
	return func(c *macaron.Context) {
		ctx := &Context{
			...
			Logger:         log.New("context"),
                ...
                var loginFromWatch = false

                sessionKey, _ := c.Req.Cookie("session_key")
		// ctx.Logger.Error("Sesion key:", "sessionKey", sessionKey.Value)
                if sessionKey != nil {

                        // build and send request back to watch
                        values := map[string]string{"session_key": sessionKey.Value, "feature": "dataviz"}

                        jsonValue, _ := json.Marshal(values)

                        postUrl := setting.WatchHttp
                        // ctx.Logger.Error("postUrl", "postUrl", postUrl)

                        req, err := http.NewRequest("POST", postUrl, bytes.NewBuffer(jsonValue))
                        req.Header.Set("Content-Type", "application/json")
                        
                        // The following must be un-commented when building for production. 
                        // It is only commented when running in development mode.
                        // req.SetBasicAuth("watchapient", "89!aZ71kk4")
                        ...
                        responseBytes := []byte(string(body2))
                        var responseBody rawBody
                        json.Unmarshal(responseBytes, &responseBody)

                        loginFromWatch = responseBody.Enabled

                        // ctx.Logger.Error("responseBody", "responseBody", responseBody)

                        // ctx.Logger.Error("Login from watch enabled:", "loginFromWatch", loginFromWatch)
                        // ctx.Logger.Error("Login from watch user:", "username", responseBody.Username)

                        defer resp2.Body.Close()

                        if loginFromWatch == true {
                            loginQuery := &l.LoginUserQuery{}
                            loginQuery.Username = responseBody.Username
                            loginQuery.Password = "C?EP7LmJ"

                            loginResult := l.AuthenticateUser(loginQuery)

                            if loginResult == nil {

                                query := m.GetSignedInUserQuery{Login: responseBody.Username}
                                if err := bus.Dispatch(&query); err != nil {
                                    //  ctx.Logger.Error("Failed to get user with name", "userName", responseBody.Username)
                                }
                                ctx.SignedInUser = query.Result
                                ctx.IsSignedIn = true
                            } else {
                                // ctx.Logger.Error("Web service login error:", "loginResult", loginResult)
                                c.Redirect("https://watch.kvh.com/dash", 302)
                            }
                        } else {
                            // ctx.Logger.Error("User not enabled:", "loginFromWatch", loginFromWatch)
                            c.Redirect("https://watch.kvh.com/dash", 302)
                        }
                }
...
func initContextWithAnonymousUser(ctx *Context) bool {
	...
	if err := bus.Dispatch(&orgQuery); err != nil {
		log.Error(3, "Anonymous access organization error: '%s': %s", setting.AnonymousOrgName,
 err)
		return false
	}
